...
int busca_largura (Grafo* g, int v1)
{
    ...
    if (verificador != 0) return 0; // VERTICE NAO EH CONECTADO A TODOS OS DEMAIS
    else return 1;
...
int busca_largura_v2 (Grafo* g, int v1){
    if (g==NULL) return -1; // GRAFO NAO EXISTE
    if (v1 < 0) return -1; // DADOS INVALIDOS
    ...
    if (verificador == 0) return 1; // VERTICE NAO SE CONECTA COM NINGUEM!
    else return 0;
